...
# Helper function to intelligently parse natural language queries
def parse_warehouse_query(query: str) -> dict:
    """Parse natural language warehouse queries into structured parameters."""
    query_lower = query.lower()

    # Default collection name - always use warehouse_inventory for warehouse queries
    ...
    filter_expr = ""
    output_fields = "*"
    limit = 50  # Increased default from 10 to show more results
    ...
    if "low stock" in query_lower:
        filter_expr = 'status == "low_stock"'
    elif "out of stock" in query_lower or "zero stock" in query_lower:
        filter_expr = 'stock_quantity == 0'
    elif "in stock" in query_lower:
        filter_expr = 'status == "in_stock"'
    ...
    if "under" in query_lower or "less than" in query_lower or "below" in query_lower:
        ...
        if price_match:
            ...
            filter_expr = f'unit_price < {price}'
    ...
    suppliers = ["Global Tech Solutions", "Fashion Forward Inc", "Fresh Foods Co",
                "Home Comfort Ltd", "Auto Parts Plus", "Active Lifestyle",
                "Knowledge Books", "Beauty Essentials", "Pro Tools Inc", "Office Depot"]
    for supplier in suppliers:
        if supplier.lower() in query_lower:
            filter_expr = f'supplier == "{supplier}"'
            break

    # Check for warehouse location queries
    locations = ["North Distribution Center", "South Warehouse", "East Coast Facility",
                "West Regional Hub", "Central Storage", "Express Fulfillment Center"]
    for location in locations:
        if location.lower() in query_lower:
            filter_expr = f'warehouse_location == "{location}"'
            break
    ...
    if "all" in query_lower:
        ...
        import re
        num_match = re.search(r'first (\d+)', query_lower)
        if num_match:
...
@mcp.tool()
async def warehouse_similarity_search(
    query_text: str,
    limit: int = 5,
    ctx: Context = None,
) -> str:
    """
    Find similar warehouse items based on text description.

    This combines text and vector search to find similar products.

    Args:
        query_text: Description of what you're looking for
        limit: Number of results to return

    Returns:
        Similar items from the warehouse
    """
    connector = ctx.request_context.lifespan_context.connector

    try:
        # Use text search on the description field
        results = await connector.search_collection(
            collection_name="warehouse_inventory",
            query_text=query_text,
            limit=limit,
            output_fields=["id", "item_name", "category", "description", "unit_price", "stock_quantity"],
        )

        output = f"Similar items for '{query_text}':\n\n"
        if not results:
        ...
        return f"Search failed: {str(e)}. Try using the warehouse_query tool instead."
...
async def warehouse_query(
    ...
    """
    Natural language query for warehouse inventory.

    Examples:
    - "show me electronics items"
    - "what items have low stock?"
    - "find items under $50"
    - "show products from Global Tech Solutions"
    - "list items in North Distribution Center"

    Args:
        query: Natural language query about warehouse inventory

    Returns:
        Query results from the warehouse inventory
    """
    connector = ctx.request_context.lifespan_context.connector

    # Parse the natural language query
    params = parse_warehouse_query(query)

    # Execute the query
    results = await connector.query_collection(
        collection_name=params["collection_name"],
        filter_expr=params["filter_expr"],
        output_fields=None if params["output_fields"] == "*" else params["output_fields"].split(","),
        limit=params["limit"],
    )

    # Format the output
    output = f"Warehouse query results for '{query}':\n\n"
    if not results:
        output += "No items found matching your query.\n"
    else:
        output += f"Found {len(results)} items:\n\n"
        for i, result in enumerate(results, 1):
            # Remove vector field from output to keep it readable
            if 'vector' in result:
                del result['vector']
            output += f"Item {i}:\n"
            for key, value in result.items():
...
async def milvus_text_search(
    ...
    Args:
        ...
        output_fields: Fields to include in results (use '*' for all fields except vectors)
        drop_ratio: Proportion of low-frequency terms to ignore (0.0-1.0)
    """
    connector = ctx.request_context.lifespan_context.connector
    ...
    if output_fields:
        if output_fields == "*":
            # Get all fields except vector fields to avoid huge responses
            try:
                info = connector.client.describe_collection(collection_name)
                output_fields_list = []
                for field in info.get('fields', []):
                    # Exclude vector fields (type 101 is FLOAT_VECTOR)
                    if field.get('type') != 101:
                        output_fields_list.append(field['name'])
            except:
                output_fields_list = None
        elif output_fields.startswith('[') and output_fields.endswith(']'):
            # Handle string representation of list: "['field1', 'field2']"
            import ast
            output_fields_list = ast.literal_eval(output_fields)
        else:
            # Handle comma-separated string: "field1,field2" or single field: "field1"
...
async def milvus_list_collections(ctx: Context = None) -> str:
    ...
    try:
        ...
        result = f"Found {len(collections)} collection(s) in Milvus:\n"
...
async def milvus_query(
    ...
    Args:
        ...
        filter_expr: Filter expression (e.g. 'age > 20' or 'category == "Electronics"')
        output_fields: Fields to include in results (use '*' for all fields except vectors)
        limit: Maximum number of results
    """
    connector = ctx.request_context.lifespan_context.connector
